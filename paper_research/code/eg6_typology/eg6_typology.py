...
CHARACTERISATION_COLS = [
    ...
    "cc_density_1200",  # Density of street network (network density)
    "cc_hill_q0_200_nw",  # Hill Number q=0 (landuse richness)
]
...
OUTPUT_DIR = "paper_research/code/eg6_typology/outputs"
TEMP_DIR = "temp/egs/eg6_typology"
...
for c in range(N_NODE_CLUSTERS):
    ...
    netw_density_values = cluster_data["cc_density_1200"].replace([np.inf, -np.inf], np.nan)
    mixed_use_values = cluster_data["cc_hill_q0_200_nw"].replace([np.inf, -np.inf], np.nan)

    density_mean = density_values.mean()
    netw_density_mean = netw_density_values.mean()
    mixed_use_mean = mixed_use_values.mean()

    CLUSTER_EXTERNAL_CHARS[c] = {
        "density_mean": density_mean,
        "netw_density_mean": netw_density_mean,
...
netw_density_ranking = sorted(range(N_NODE_CLUSTERS), key=lambda c: CLUSTER_EXTERNAL_CHARS[c]["netw_density_mean"])
...
sorted_netw_density = [CLUSTER_EXTERNAL_CHARS[c]["netw_density_mean"] for c in netw_density_ranking]
sorted_labels_walk = [c + 1 for c in netw_density_ranking]
bars = ax.barh(range(N_NODE_CLUSTERS), sorted_netw_density, color=colors_walk)
ax.set_yticks(range(N_NODE_CLUSTERS))
ax.set_yticklabels([f"Cluster {label}" for label in sorted_labels_walk], fontsize=9)
ax.set_xlabel("Street Network Density (density)", fontsize=10, weight="bold")
ax.set_title("Clusters Ranked by\nNetwork Density", fontsize=11, weight="bold")
ax.grid(True, alpha=0.3, axis="x")
# Put labels inside bars on the right side
for i, (c, val) in enumerate(zip(netw_density_ranking, sorted_netw_density)):
    if not np.isnan(val) and val > 0:
...
for bounds_fid in nodes_df_clean["bounds_fid"].unique():
    ...
    profile = {
        ...
        "mixed_use_mean": city_nodes["cc_hill_q0_200_nw"].mean(),
        "netw_density_mean": city_nodes["cc_density_1200"].mean(),
    }
...
for c in range(N_NODE_CLUSTERS):
    ...
    netw_density_mean = ext_chars.get("netw_density_mean", 0)
    mixed_use_mean = ext_chars.get("mixed_use_mean", 0)

    readme_content += f"| {c + 1} | {n_nodes:,} | {pct:.1f}% | {density_mean:.0f} | {netw_density_mean:.2f} | {mixed_use_mean:.2f} |\n"
...
# %%
"""
## Step 12: LaTeX Table Generation
"""
...
latex_clusters = [
    ...
    r"  \toprule",
    r"  Cluster & Nodes & \% Total & Pop Density & Network Density & Mixed Use \\",
    r"  \midrule",
]
for c in range(N_NODE_CLUSTERS):
    cluster_mask = nodes_df_clean["node_cluster"] == c
    n_nodes = cluster_mask.sum()
    pct = n_nodes / len(nodes_df_clean) * 100
    chars = CLUSTER_EXTERNAL_CHARS[c]
    latex_clusters.append(
        f"  {c + 1} & {n_nodes:,} & {pct:.1f} & {chars['density_mean']:.0f} & {chars['netw_density_mean']:.1f} & {chars['mixed_use_mean']:.2f} \\\\"
    )
latex_clusters.extend([r"  \bottomrule", r"\end{tabular}"])

with open(output_path / "table_cluster_characteristics.tex", "w") as f:
    f.write("\n".join(latex_clusters))
print("  Saved: table_cluster_characteristics.tex")

# Table 2: Top Countries by Cluster
for c in range(min(3, N_NODE_CLUSTERS)):  # Just first 3 clusters for brevity
    top_countries = country_profiles_clean.nlargest(5, f"pct_cluster_{c}")
    latex_country = [
        r"\begin{tabular}{@{}l r r r@{}}",
        r"  \toprule",
        f"  Country & Cities & Dominant & \\% Cluster {c + 1} \\\\",
        r"  \midrule",
    ]
