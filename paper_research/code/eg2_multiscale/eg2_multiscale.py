...
OUTPUT_DIR = "paper_research/code/eg2_multiscale/outputs"
TEMP_DIR = "temp/egs/eg2_multiscale"
...
# %%
"""
## Step 7: LaTeX Table Generation
"""

print("\nSTEP 7: LaTeX Table Generation")

# Extract summary statistics from the dictionary
mean_dist_green = summary_stats["green_mean_dist"]
median_dist_green = summary_stats["green_median_dist"]
pct_within_400_green = summary_stats["green_pct_within_400m"]
mean_dist_trees = summary_stats["trees_mean_dist"]
median_dist_trees = summary_stats["trees_median_dist"]
pct_within_400_trees = summary_stats["trees_pct_within_400m"]

# Table: Summary statistics
latex_summary = [
    r"\begin{tabular}{@{}l r r r@{}}",
    r"  \toprule",
    r"  Metric & Green Blocks & Tree Canopy \\",
    r"  \midrule",
    f"  Mean Distance (m) & {mean_dist_green:.1f} & {mean_dist_trees:.1f} \\\\",
    f"  Median Distance (m) & {median_dist_green:.1f} & {median_dist_trees:.1f} \\\\",
    f"  \\% Within 400m & {pct_within_400_green:.1f} & {pct_within_400_trees:.1f} \\\\",
    r"  \midrule",
    f"  Cities with $\\rho < 0$ & {green_negative} & {trees_negative} \\\\",
    f"  Cities with $\\rho > 0$ & {green_positive} & {trees_positive} \\\\",
    r"  \bottomrule",
    r"\end{tabular}",
]

with open(output_path / "table_summary.tex", "w") as f:
    f.write("\n".join(latex_summary))
print("  Saved: table_summary.tex")

# Table: Strongest correlations (green blocks)
latex_green_corr = [
    r"\begin{tabular}{@{}l l r@{}}",
    r"  \toprule",
    r"  City & Country & $\rho$ \\",
    r"  \midrule",
    r"  \multicolumn{3}{@{}l}{\textit{Negative (denser = closer to parks)}} \\",
]
for _, row in strongest_green_neg.head(3).iterrows():
    city = str(row["city_label"]).replace("&", r"\&")
    latex_green_corr.append(f"  {city} & {row['country']} & {row['green_corr']:.3f} \\\\")
latex_green_corr.append(r"  \midrule")
latex_green_corr.append(r"  \multicolumn{3}{@{}l}{\textit{Positive (denser = farther from parks)}} \\")
for _, row in strongest_green_pos.head(3).iterrows():
    city = str(row["city_label"]).replace("&", r"\&")
    latex_green_corr.append(f"  {city} & {row['country']} & {row['green_corr']:.3f} \\\\")
latex_green_corr.extend([r"  \bottomrule", r"\end{tabular}"])

with open(output_path / "table_green_correlations.tex", "w") as f:
    f.write("\n".join(latex_green_corr))
print("  Saved: table_green_correlations.tex")

# Table: Strongest correlations (tree canopy)
latex_trees_corr = [
    r"\begin{tabular}{@{}l l r@{}}",
    r"  \toprule",
    r"  City & Country & $\rho$ \\",
    r"  \midrule",
    r"  \multicolumn{3}{@{}l}{\textit{Negative (denser = closer to trees)}} \\",
]
for _, row in strongest_trees_neg.head(3).iterrows():
    city = str(row["city_label"]).replace("&", r"\&")
    latex_trees_corr.append(f"  {city} & {row['country']} & {row['trees_corr']:.3f} \\\\")
latex_trees_corr.append(r"  \midrule")
latex_trees_corr.append(r"  \multicolumn{3}{@{}l}{\textit{Positive (denser = farther from trees)}} \\")
for _, row in strongest_trees_pos.head(3).iterrows():
    city = str(row["city_label"]).replace("&", r"\&")
    latex_trees_corr.append(f"  {city} & {row['country']} & {row['trees_corr']:.3f} \\\\")
latex_trees_corr.extend([r"  \bottomrule", r"\end{tabular}"])

with open(output_path / "table_trees_correlations.tex", "w") as f:
    f.write("\n".join(latex_trees_corr))
print("  Saved: table_trees_correlations.tex")
