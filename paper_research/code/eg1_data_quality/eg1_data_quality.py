...
def aggregate_grid_stats(
    ...
    for bounds_fid in tqdm(grids_in_cities["bounds_fid"].unique(), desc="Processing cities"):
        ...
        try:
            ...
            print(f"  City {bounds_fid}: Processed {len(city_grids)} grids, {len(places_gdf)} total POIs")

        except Exception as e:
            print(f"  City {bounds_fid}: Error processing - {e}")
            continue
...
# %%
"""
## Step 8: LaTeX Table Generation

Generate LaTeX tables for inclusion in demonstrators.tex paper.
"""
...
latex_model_performance = [
    r"\begin{tabular}{@{}L{4cm}R{1.2cm}R{1.5cm}R{1.5cm}R{1.5cm}@{}}",
    r"  \toprule",
    r"  \textbf{Category} & \textbf{R\textsuperscript{2}} & \textbf{Local} & \textbf{Intermed.} & \textbf{Large} \\",
    r"  \midrule",
]

# Sort by R² descending for clean presentation
sorted_cats = sorted(
    [cat for cat in poi_categories if not np.isnan(regression_results[cat]["r2"])],
    key=lambda c: regression_results[c]["r2"],
    reverse=True,
)
...
for i, cat in enumerate(sorted_cats):
    ...
    imp_int = regression_results[cat]["importance_intermediate"]
    imp_large = regression_results[cat]["importance_large"]
    name = category_names[cat].replace("&", r"\&")
    latex_model_performance.append(f"  {name} & {r2:.2f} & {imp_local:.2f} & {imp_int:.2f} & {imp_large:.2f} \\\\")
    # Add midrule after top 5 and before bottom 2
    if i == 4 or i == 8:
        latex_model_performance.append(r"  \midrule")

latex_model_performance.extend([r"  \bottomrule", r"\end{tabular}"])

with open(output_path / "table_model_performance.tex", "w") as f:
    f.write("\n".join(latex_model_performance))
print("  Saved: table_model_performance.tex")

# Table 2: Consistently Undersaturated Cities (top 10)
undersaturated = city_gdf[city_gdf["between_category_quadrant"] == "Consistently Undersaturated"].sort_values(
    "overall_z_mean_category"
)
latex_undersaturated = [
    r"\begin{tabular}{@{}l l r r@{}}",
    r"  \toprule",
    r"  City & Country & Mean Z & Std \\",
    r"  \midrule",
]
for _, row in undersaturated.head(10).iterrows():
    city = str(row.get("label", "Unknown")).replace("&", r"\&")
    country = str(row.get("country", ""))
    mean_z = row.get("overall_z_mean_category", 0)
    std_z = row.get("overall_z_std_category", 0)
    latex_undersaturated.append(f"  {city} & {country} & {mean_z:.3f} & {std_z:.3f} \\\\")
latex_undersaturated.extend([r"  \bottomrule", r"\end{tabular}"])

with open(output_path / "table_undersaturated.tex", "w") as f:
    f.write("\n".join(latex_undersaturated))
print("  Saved: table_undersaturated.tex")

# Table 3: Consistently Saturated Cities (top 10)
saturated = city_gdf[city_gdf["between_category_quadrant"] == "Consistently Saturated"].sort_values(
    "overall_z_mean_category"
)
latex_saturated = [
    r"\begin{tabular}{@{}l l r r@{}}",
...
for _, row in saturated.head(10).iterrows():
    city = str(row.get("label", "Unknown")).replace("&", r"\&")
    country = str(row.get("country", ""))
    mean_z = row.get("overall_z_mean_category", 0)
    std_z = row.get("overall_z_std_category", 0)
    latex_saturated.append(f"  {city} & {country} & {mean_z:.3f} & {std_z:.3f} \\\\")
latex_saturated.extend([r"  \bottomrule", r"\end{tabular}"])

with open(output_path / "table_saturated.tex", "w") as f:
    f.write("\n".join(latex_saturated))
print("  Saved: table_saturated.tex")
