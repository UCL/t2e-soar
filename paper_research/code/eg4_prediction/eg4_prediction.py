...
OUTPUT_DIR = "paper_research/code/eg4_prediction/outputs"
TEMP_DIR = "temp/egs/eg4_prediction"
...
for cat_name in POI_CATEGORY_NAMES:
    ...
    # Table: Best and Worst Predicted Cities
    latex_cities = [
        r"\begin{tabular}{@{}l r r r r@{}}",
        r"  \toprule",
        r"  City & R\textsuperscript{2} & MAE & RMSE & Nodes \\",
        r"  \midrule",
    ]
    for _, row in city_agg.head(5).iterrows():
        city = str(row["city_name"]).replace("&", r"\&")
        latex_cities.append(
            f"  {city} & {row['r2']:.3f} & {row['mae']:.3f} & {row['rmse']:.3f} & {int(row['n_nodes']):,} \\\\"
        )
    latex_cities.append(r"  \ldots & & & & \\")
    for _, row in city_agg.tail(4).iloc[::-1].iterrows():
        city = str(row["city_name"]).replace("&", r"\&")
        latex_cities.append(
            f"  {city} & {row['r2']:.3f} & {row['mae']:.3f} & {row['rmse']:.3f} & {int(row['n_nodes']):,} \\\\"
        )
    latex_cities.extend([r"  \bottomrule", r"\end{tabular}"])

    safe_name = cat_name.replace(" ", "_").replace("&", "and")
    with open(output_path / f"table_{safe_name}_cities.tex", "w") as f:
        f.write("\n".join(latex_cities))
    print(f"  Saved: table_{safe_name}_cities.tex")

    # Table: Feature Importance (top 10)
    latex_importance = [
        r"\begin{tabular}{@{}l l r l r@{}}",
        r"  \toprule",
        r"  \multicolumn{2}{c}{" + cat_name.replace("&", r"\&") + r"} & & \multicolumn{2}{c}{Other} \\",
        r"  \cmidrule{1-2} \cmidrule{4-5}",
        r"  Feature & Importance & & Feature & Importance \\",
        r"  \midrule",
    ]
    importance = model_info["feature_importance"].head(10)
    for i, (_, row) in enumerate(importance.iterrows()):
        feature = row["feature"].replace("_", " ").title()
        latex_importance.append(f"  {feature} & {row['importance']:.3f} & & & \\\\")
    latex_importance.extend([r"  \bottomrule", r"\end{tabular}"])

    with open(output_path / f"table_{safe_name}_importance.tex", "w") as f:
        f.write("\n".join(latex_importance))
    print(f"  Saved: table_{safe_name}_importance.tex")

# Combined feature importance table for both categories
latex_combined = [
    r"\begin{tabular}{@{}l l r l r@{}}",
    r"  \toprule",
    r"  \multicolumn{2}{c}{Eat \& Drink} & & \multicolumn{2}{c}{Business \& Services} \\",
    r"  \cmidrule{1-2} \cmidrule{4-5}",
    r"  Feature & Importance & & Feature & Importance \\",
    r"  \midrule",
]

eat_imp = models[POI_CATEGORY_NAMES[0]]["feature_importance"].head(10)
bus_imp = models[POI_CATEGORY_NAMES[1]]["feature_importance"].head(10)

for i in range(10):
    eat_row = eat_imp.iloc[i]
    bus_row = bus_imp.iloc[i]
    eat_feat = eat_row["feature"].replace("_", " ").title()
    bus_feat = bus_row["feature"].replace("_", " ").title()
    latex_combined.append(
        f"  {eat_feat} & {eat_row['importance']:.3f} & & {bus_feat} & {bus_row['importance']:.3f} \\\\"
    )

latex_combined.extend([r"  \bottomrule", r"\end{tabular}"])

with open(output_path / "table_feature_importance.tex", "w") as f:
    f.write("\n".join(latex_combined))
print("  Saved: table_feature_importance.tex")
