...
def load_overture_infrastructure(bounds_in_path: str, cities_data_out_dir: str) -> None:
    """ """
    ...
    # loop through bounds and load infrastructure
    for bounds_fid, bounds_row in tqdm(bounds_gdf.iterrows(), total=len(bounds_gdf)):
        ...
        infrast_gdf = loaders.load_infrastructure(bounds_row.geometry, 3035)
        infrast_gdf["bounds_fid"] = bounds_fid
        infrast_gdf.to_file(output_path, driver="GPKG", layer="infrastructure", overwrite=True)


if __name__ == "__main__":
    """
    python -m src.data.ingest_overture_infrast temp/datasets/boundaries.gpkg temp/cities_data
    """
    if True:
        parser = argparse.ArgumentParser(description="Load overture infrastructure.")
