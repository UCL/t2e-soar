...
def download_location(
    ...
    # NETWORK
    _nodes_gdf, _edges_gdf, clean_edges_gdf = overture_data.load_network(bounds_geom_wgs, crs)
    ...
    # BUILDINGS
    buildings_gdf = overture_data.load_buildings(bounds_geom_wgs, crs)
    buildings_gdf.to_file(out_path / f"{location_key}_buildings.gpkg")

    # INFRASTRUCTURE
    infrast_gdf = overture_data.load_infrastructure(bounds_geom_wgs, crs)
    infrast_gdf.to_file(out_path / f"{location_key}_infrastructure.gpkg")

    # PLACES
    places_gdf = overture_data.load_places(bounds_geom_wgs, crs)
