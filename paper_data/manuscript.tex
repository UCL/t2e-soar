...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}L{3.2cm}L{8.3cm}@{}}
    ...
    \midrule
    \textbf{Related research article} & \todo{cite demonstrators paper} \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{@{}L{4.5cm}L{7.5cm}@{}}
    ...
    \midrule
    Data quality filtering & Identify cities with lower than expected POI saturation via regression residuals \\
    \addlinespace
    Multi-scale analysis & Compare within-city vs.\ between-city relationships \\
    \addlinespace
    Access gap identification & Locate areas with poor access to services or amenities such as education or transport \\
    \addlinespace
    Predictive modelling & Train cross-city models for land-use intensity prediction \\
    \addlinespace
    Benchmarking & Benchmark and rank cities by walkable access to amenities \\
    \addlinespace
    Classification & Cluster neighbourhoods into characteristic urban forms \\
    \addlinespace
    Site selection & Filter locations meeting target development criteria \\
    \bottomrule
  \end{tabular}
\end{table}
